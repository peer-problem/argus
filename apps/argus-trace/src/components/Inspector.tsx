...
export function Inspector({ event }: { event: ArgusEvent | null }) {
  ...
  return (
    <aside className="inspector" aria-label="Event inspector">
      <Tabs.Root className="inspector-tabs" value={mode} onValueChange={(next) => setMode(next as "normalized" | "raw")}>
        <div className="inspector-head">
          <div><p className="eyebrow">Event evidence</p><h2>{event.kind.replace(".", " / ")}</h2></div>
          <Tabs.List className="peer-tabs-list" aria-label="Evidence representation">
            <Tabs.Tab className="peer-tab" value="normalized">Normalized</Tabs.Tab>
            <Tabs.Tab className="peer-tab" value="raw">Raw</Tabs.Tab>
            <Tabs.Indicator className="peer-tabs-indicator" />
          </Tabs.List>
        </div>
        <Tabs.Panel className="inspector-panel" value="normalized">
        ...
        <Tabs.Panel className="inspector-panel" value="raw">
